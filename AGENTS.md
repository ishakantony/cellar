...
- `apps/shell` (formerly `apps/web`) — React 19 + Vite SPA. React Router v7, TanStack Query, Hono RPC client, Zustand, Tailwind v4, Storybook.
...
- New tables go into `apps/api/src/db/schema.ts`, then run `bun run db:generate` to produce a Drizzle migration. Boot runs migrations automatically.
...
- Frontend data access goes through TanStack Query hooks in `apps/shell/src/hooks/{queries,mutations}/`. Don't fetch from page components directly.
- Forms use `react-hook-form` + `@hookform/resolvers` + Zod schemas from `@cellar/shared`.
- Stories live next to their component as `*.stories.tsx`. Storybook config is at `apps/shell/.storybook`.
...
bun install
cp .env.example .env            # fill in BETTER_AUTH_SECRET (>= 32 chars) — single root .env is read by both shell and api
bun dev                         # runs shell on :5200 and api on :5201 in parallel
```

Vite proxies `/api` and `/.well-known` to the Hono backend. In production, Hono serves the built SPA from `apps/shell/dist` on the same origin.
