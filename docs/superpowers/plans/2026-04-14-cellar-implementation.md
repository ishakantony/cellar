  ...
  return (
    <>
      ...
      <p className="mt-6 text-center text-xs text-outline">
        ...
        <Link href="/sign-up" className="text-primary hover:text-primary-dim transition-colors">
          Sign up
        </Link>
      </p>
    </>
  ...
  return (
    <>
      ...
      <p className="mt-6 text-center text-xs text-outline">
        ...
        <Link href="/sign-in" className="text-primary hover:text-primary-dim transition-colors">
          Sign in
        </Link>
      </p>
    </>
...
const TYPE_CONFIG: Record<AssetType, { icon: typeof FileCode; iconWrap: string; label: string }> = {
  SNIPPET: {
    icon: Braces,
      ...
      <div className="relative" ref={menuRef}>
        ...
        {menuOpen && (
          <div className="absolute right-0 top-full mt-1 w-40 bg-surface-container-high rounded-lg shadow-xl border border-white/10 py-1 z-50">
            ...
            >
              {asset.pinned ? <PinOff className="h-3.5 w-3.5" /> : <Pin className="h-3.5 w-3.5" />}
...
import { Code, Terminal, Link as LinkIcon, StickyNote, Image, FileText } from 'lucide-react';
import { AssetType } from '@/generated/prisma';
...
export function QuickActions({ onAction }: { onAction: (type: AssetType) => void }) {
  return (
        ...
        >
          ...
          <span
            className={`text-[10px] font-bold uppercase tracking-widest leading-tight ${
              action.primary ? 'text-on-primary-container' : 'text-on-surface-variant'
            }`}
          >
            {action.label}
          </span>
    ...
    >
      ...
      >
        <div className="flex items-start gap-4 mb-5">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-100">Delete &ldquo;{title}&rdquo;?</h3>
...
import { X, Pencil, Trash2, Save, Copy, ExternalLink, Eye, Code, Download } from 'lucide-react';
import {
  FileCode,
  Terminal,
  Link as LinkIcon,
...
export function AssetDrawer({
  ...
  const [type, setType] = useState<AssetType>(asset?.type ?? defaultType ?? 'SNIPPET');
  ...
  async function handleSave() {
    ...
    if (mode === 'create') {
      await createAsset({
        type,
        title,
        content: ['SNIPPET', 'PROMPT', 'NOTE'].includes(type) ? content : undefined,
  ...
  const editorLanguage = type === 'SNIPPET' ? language : isMarkdown ? 'markdown' : 'plaintext';
            ...
            <div className="flex-1 overflow-hidden flex flex-col px-6 pb-6 pt-2">
              ...
              >
                ...
                <div className="flex items-center gap-2 px-4 h-9 bg-surface-bright border-b border-white/5 rounded-t-xl select-none shrink-0">
                  ...
                  <span className="text-[10px] text-outline/60 font-mono">{editorLanguage}</span>
                </div>
              ...
              {mode === 'create' || (!asset?.filePath && editing) ? (
                ...
                <>
                  ...
                  <div className="text-center space-y-1">
                    <p className="text-sm font-semibold text-slate-200">{asset.fileName}</p>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-1 overflow-x-auto">
          {TYPE_TABS.map(tab => (
            <button
              key={tab.label}
              onClick={() => updateParams({ type: tab.value, q: null })}
        ...
        {assets.length === 0 && (
          <div className="py-16 text-center">
            <p className="text-xs text-outline">
              {searchQuery ? 'No results found.' : 'No assets yet. Create one from the dashboard!'}
            </p>
          </div>
        )}
      </div>
...
export default async function AssetsPage({
  ...
  return <AssetsClient assets={assets} currentType={type} currentSort={sort} searchQuery={q} />;
}
```
