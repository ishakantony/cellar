...
// ---------------------------------------------------------------------------
// Module-level cache: track which features have been loaded so subsequent
// palette opens are instant.
// ---------------------------------------------------------------------------

const loadedModules = new Map<string, FeatureModule>();

async function ensureModulesLoaded(): Promise<void> {
  const entries = registry.list();
  await Promise.all(
    entries.map(async entry => {
      if (!loadedModules.has(entry.manifest.id)) {
        try {
          const mod = await entry.load();
          loadedModules.set(entry.manifest.id, mod);
        } catch {
          // Ignore load failures — the palette will degrade gracefully
        }
      }
    })
  );
...
export function CommandPalette() {
  ...
  const activeFeatureId = useMemo(() => {
    const entries = registry.list();
    // Sort descending by basePath length to match the most specific prefix first
  ...
  useEffect(() => {
    if (open) {
      ...
      // Trigger module prefetch on open
      setModulesLoading(true);
      void ensureModulesLoaded().finally(() => setModulesLoading(false));
    }
  }, [open]);
  ...
  // Build named providers from loaded modules
  const namedProviders = useMemo((): NamedPaletteProvider[] => {
    const providers: NamedPaletteProvider[] = [];
    for (const [id, mod] of loadedModules) {
      if (mod.paletteProvider) {
        providers.push({ id, provider: mod.paletteProvider });
      }
    }
    return providers;
  }, [modulesLoading]); // eslint-disable-line react-hooks/exhaustive-deps -- re-compute when modules finish loading

  // Fetch recents when palette opens with empty query
  useEffect(() => {
    if (!open) return;
    ...
    async function fetchRecents() {
      const results = await Promise.allSettled(
        namedProviders
          .filter(p => typeof p.provider.getRecent === 'function')
          .map(p => p.provider.getRecent!())
      );
      if (cancelled) return;
  ...
  }, [open, query, namedProviders]);

  // Search providers with debounce when query is non-empty
  useEffect(() => {
    if (!open) return;
    ...
    // Mark all groups as loading immediately
    const initialGroups: ItemGroup[] = namedProviders.map(p => ({
      id: p.id,
      label: registry.list().find(e => e.manifest.id === p.id)?.manifest.label ?? p.id,
      items: [],
      status: 'loading',
    }));
    ...
    const debounceId = setTimeout(async () => {
      const results = await aggregatePaletteResults(trimmed, controller.signal, namedProviders);
      ...
      setItemGroups(
        results.map(r => ({
          id: r.providerId,
          label:
            registry.list().find(e => e.manifest.id === r.providerId)?.manifest.label ??
            r.providerId,
          items: r.items,
          status: r.status,
        }))
  ...
  const isSearching = query.trim().length > 0 && sortedItemGroups.some(g => g.status === 'loading');
  const hasNoResults =
    query.trim().length > 0 &&
              ...
              {isSearching ? (
                ...
                <>
                  ...
                  {sortedItemGroups
                    ...
                    .map(group => (
                      <ItemGroupSection key={group.id} group={group} onSelect={handleSelectItem} />
                    ))}
